...
CREATE OR ALTER PROC PR_DEPARTMENT_DETAIL
@DepartmentName VARCHAR(100)
AS
BEGIN
	SELECT FirstName,LastName FROM PERSON JOIN Department
	ON PERSON.DepartmentID = Department.DepartmentID
	WHERE DepartmentName = @DepartmentName
END

EXEC PR_DEPARTMENT_DETAIL 'ADMIN'
...
CREATE OR ALTER PROC PR_WORKER_DETAIL
@DepartmentName VARCHAR(100),
@DesignationName VARCHAR(100)
AS
BEGIN
	SELECT FirstName,Salary,JoiningDate,DepartmentName FROM PERSON JOIN Department
	ON PERSON.DepartmentID = Department.DepartmentID
	JOIN Designation
	ON PERSON.DesignationID = Designation.DesignationID
...
EXEC PR_DEPARTMENT_DETAIL 'RAHUL'

--QUESTION-8

--Create Procedure which displays department wise maximum, minimum & total salaries.

CREATE OR ALTER PROC PR_DEPARTMENT_WITH_GROUPbY
@DepartmentName VARCHAR(100)
AS
BEGIN
	SELECT MAX(Salary),MIN(Salary),SUM(Salary) FROM PERSON JOIN Department
	ON PERSON.DepartmentID = Department.DepartmentID
	WHERE DepartmentName=@DepartmentName
	GROUP BY DepartmentName
END

EXEC PR_DEPARTMENT_WITH_GROUPbY 'ADMIN'

--QUESTION-9

--Create Procedure which displays designation wise average & total salaries. 

CREATE OR ALTER PROC PR_DESIGNATION_WITH_GROUPbY
@DesignationName VARCHAR(100)
AS
BEGIN
	SELECT AVG(Salary),SUM(Salary) FROM PERSON JOIN Designation
	ON PERSON.DesignationID = Designation.DesignationID
	WHERE DesignationName=@DesignationName
	GROUP BY DesignationName
END

EXEC PR_DESIGNATION_WITH_GROUPbY 'CEO'
